...
OIP=`ifconfig uap0 | awk -F ":" '/inet addr/ {print $2}' | awk '{print $1}'`
set1=`ifconfig uap0 | awk -F ":" '/inet addr/ {print $2}' | awk '{print $1}' | awk -F '[.]' '{print $1}'`
set2=`ifconfig uap0 | awk -F ":" '/inet addr/ {print $2}' | awk '{print $1}' | awk -F '[.]' '{print $2}'`
set3=`ifconfig uap0 | awk -F ":" '/inet addr/ {print $2}' | awk '{print $1}' | awk -F '[.]' '{print $3}'`
...
for i in `grep -rl ''$OIP'' /root/k2.6.39.4/init_setup8787.sh 2> /dev/null`; do sed -i 's/'$OIP'/'$IP'/' $i; done
for i in `grep -rl ''$ONET'' /root/k2.6.39.4/init_setup8787.sh 2> /dev/null`; do sed -i 's/'$ONET'/'$NET'/' $i; done
ifconfig uap0 $IP/24
#Set DHCP scripts
