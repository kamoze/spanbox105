...
if(isset($_POST['upl'] )) {
 if(file_exists("/etc/vpnmgt/wwwmgt/".$_FILES["file"]["name"])) {
   $_SESSION['error'] = 1;
 } 
 ...
 $i = move_uploaded_file($_FILES["file"]["tmp_name"], "/etc/vpnmgt/wwwmgt/".$filename); 
if($i) {
...
if(isset($_POST['s-userpass'])) {
   if(empty($_POST['s-user']) || empty($_POST['s-pass'])) {
     ...
     $_SESSION['user_error'] = 0;
   $command = 'sudo  /usr/bin/spanbox.sh add_wwwpass '.$_POST['s-user'].' '.$_POST['s-pass'].' '.$_POST['s-server'];
  exec($command);
  }
}
...
if($_POST['trunk_start'] == "yes") {
   exec('sudo  /usr/bin/spanbox.sh vpn_restart2');
  print "<script type=\"text/javascript\">";
...
if($_POST['trunk_stop'] == "yes") {
   exec('sudo  /usr/bin/spanbox.sh vpn_stop2');
  print "<script type=\"text/javascript\">";
...
<h3 style="margin-left:15px;">VPN-CONNECT - INTERNET</h3>
<tr>
  ...
  <td  width ="350 px" style="border:0px solid green;
    ...
    <a href="vpn_config.php" style="text-decoration:none;"> VPN-CONNECT - VVPS</a>
        </span>
    </form>
  </td>
  <td width="10 px"></td>
...
<?php  $i = exec('sudo /usr/local/vpnclient/vpncmd localhost /CLIENT /CMD AccountStatusGet WWW | grep "Session Established" | awk "{print $1}"');
     if(strstr($i, "Session")) {
...
<tr>
  <td width="200 px" style="text-align:center; color:;">VPN-INTERNET Parameters </td>
  <td width="200 px" style="text-align:center; color:;"></td>

<br/>
<br/>
<form method = "post" action="vpn_config_pc.php" name="s-user_pass" id="s-user_pass" >
<tr>
...
<tr>
  ...
  <td  width ="350 px" style="border:0px solid green; 
                              ...
                              filter:alpha(opacity=60); /* For IE8 and earlier */
                              background-color:;">
    <form method = "post" action="vpn_config_pc.php" name="start" id="start">
...
<tr>
  ...
  <td  width ="350 px" style="border:0px solid red;
                              ...
                              filter:alpha(opacity=60); /* For IE8 and earlier */
                              background-color:;">
    <form method = "post" action="vpn_config_pc.php" name="stop" id="stop">
