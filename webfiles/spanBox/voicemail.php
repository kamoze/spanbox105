...
if(isset($_POST['voicemail'])) {
   if(empty($_POST['vuser']) || empty($_POST['vpass']) || empty($_POST['vhost']) || empty($_POST['vcrypt'])) {
     $_SESSION['user_error'] = 1;
   } else {
     $_SESSION['user_error'] = 0;
   $command = 'sudo  /usr/bin/spanbox.sh add_voicemail '.$_POST['vuser'].' '.$_POST['vpass'].' '.$_POST['vhost'].' '.$_POST['vcrypt'];
  exec($command);
  }
}
...
<h3 style="margin-left:15px;">VoiceMail CONFIG</h3>
<br>
<?php 
if($_SESSION['error'] == 1) {
...
<tr>
  <td width="200 px" style="text-align:center; color:;">Voicemail Mail Cfg </td>
  <td width="200 px" style="text-align:center; color:;">Insert Mail Server Credentials Below</td>


<form method = "post" action="voicemail.php" name="s-user_pass" id="s-user_pass" >
<tr>
  ...
  <td  width ="350 px" style="background-color:; 
            ...
            Mail-Host <input type="text" name="vhost" />
  </td>
  <td width="10 px"></td>
</tr>
...
<tr>
  ...
  <td  width ="350 px" style="background-color:; 
                              ...
                              filter:alpha(opacity=60); /* For IE8 and earlier */
                              border:0px solid green;"> 
            Mail-Username <input type="text" name="vuser" />
  </td>
  <td width="10 px"></td>
</tr>
...
<tr>
  ...
  <td  width ="350 px" style="background-color:; 
                              ...
                              filter:alpha(opacity=60); /* For IE8 and earlier */
                              border:0px solid green;"> 
            Use-Encryption <input type="text" name="vcrypt" />
  </td>
  <td width="10 px"></td>
</tr>
...
<tr>
  ...
  <td  width ="350 px" style="border:0px solid green; background-color:;
            ...
            Password <input type="password" name="vpass">
  </td>
  <td width="10 px"></td>
</tr>
...
<td style="background-color:;
                              opacity:0.6;
                              filter:alpha(opacity=60); ">
<input type="hidden" name="voicemail" value="1">
...
<tr>
  ...
  <td  width ="350 px" style="border:0px solid green;
    ...
    <a href="clear_voicemail.php" style="text-decoration:none;"> Clear Voicemail Directory</a>
	</span>
    </form>
  </td>
  <td width="10 px"></td>
