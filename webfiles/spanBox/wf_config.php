...
if(isset($_POST['tg_sc_gen'])) {
  $command = 'sudo  /usr/bin/spanbox.sh target_dnsserver '.$_POST['tg_sc_gen'].' '.$_POST['port'].' '.$_POST['dev'].' '.$_POST['addr'].' '.$_POST['routing'];
  exec($command);
  print "<script type=\"text/javascript\">"; 
  echo 'alert("Filter Parameters Applied")'; 
  print "</script>";  
}
...
if($_POST['restart'] == "yes") {
   exec('sudo  /usr/bin/spanbox.sh wf_start2');
  print "<script type=\"text/javascript\">"; 
...
if($_POST['trunk_stop'] == "yes") {
   exec('sudo  /usr/bin/spanbox.sh wf_stop');
  print "<script type=\"text/javascript\">";
...
<h3 style="margin-left:15px;">Web Filter - Managed</h3>
<tr>
...
<?php  $i = exec('sudo /usr/local/vpnclient/vpncmd localhost /CLIENT /CMD AccountStatusGet DNS | grep "Session Established" | awk "{print $1}"');
     if(strstr($i, "Session")) {
...
<form method = "post" action="wf_config.php" name="vpn_opt" id="vpn_opt" >
<br/><br/>
<!---
<tr>
  <td width="200 px" style="text-align:left;"></td>
...
--->
<tr>
  <td width="500 px" style="text-align:center; color:;">Filter ID</td>
  <td  width ="500 px" style="border:0px solid green; 
                              padding-top:15px; 
                              background-color:;
                              opacity:0.6;
                              filter:alpha(opacity=60); /* For IE8 and earlier */
                              padding-bottom: 1em;"> 
    <form method = "post" action="wf_config.php" name="target" id="target">
...
-->
    ...
    <a href="javascript:submitform2()" style="text-decoration:none; margin-top:-9px;"> Update Parameters</a> 
	</span>
   </form></td>
  <td width="10 px"></td>
</tr>

<form method = "post" action="wf_config.php" name="s-user_pass" id="s-user_pass" >
<!---
<tr>
...
<tr>
  ...
  <td  width ="350 px" style="border:0px solid green; 
                              ...
                              filter:alpha(opacity=60); /* For IE8 and earlier */
                              background-color:;">
    <form method = "post" action="wf_config.php" name="start" id="start">
...
<tr>
  ...
  <td  width ="350 px" style="border:0px solid red;
                              ...
                              filter:alpha(opacity=60); /* For IE8 and earlier */
                              background-color:;">
    <form method = "post" action="wf_config.php" name="stop" id="stop">
